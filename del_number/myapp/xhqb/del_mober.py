...
def base_check_update(phone=None, identity_no=None):
    ...
    up_date=locahost_mober()
    data_base=cif_customer_base.base_check(phone,identity_no)
    # print(data_base)
    # print(up_date)
    if data_base["success"]==True:
        """修改cifdb.t_customer_base数据"""
        cif_customer_base.update_base(data_base["cid"],up_date["up_identity_no"],up_date["up_name"],up_date["up_phone"])
    #     update_base(cid, up_identity_no, up_name, up_phone)
        ...
        if info_base["success"]==True:
            maincifdb_customer_info.update_info(id=info_base["id"], up_identity_no=up_date["up_identity_no"], up_name=up_date["up_name"], up_phone=up_date["up_phone"])
        ...
        if credentials_base["success"] == True:
            cifdb_customer_credentials.credentials_update(credentials_base["id"],up_identity_no=up_date["up_identity_no"])

        """修改cifdb.t_customer_md5数据"""
        cif_customer_md5.update_md5(up_date["up_identity_no"],up_phone=up_date["up_phone"])

        """修改cifdb.t_customer_partner数据"""
        cif_customer_partner.partner_check(data_base["cid"])

        inst_names(up_identity_no=up_date["up_identity_no"], up_phone=up_date["up_phone"])
        return "修改成功"
    else:
        return "修改失败!"
