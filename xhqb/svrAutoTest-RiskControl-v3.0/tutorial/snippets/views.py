...
def jenkins_job_triger(request):
    ...
    if request.method == 'POST':
        ...
        cmdStr = 'curl -s -I -XPOST  "http://10.3.208.42:8080/jenkins/job/'+ jobName +'/buildWithParameters?'+params+'" --user fengyu:Fengyu@1qa'
        output = cmd(cmdStr)
        print(output)
        return HttpResponse(content=output[0].split(b'\n')[2].split(b'/')[-2])
    else:
